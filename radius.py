...
def find_closest_entity(player_file, entity_files):
    ...
    # Iterate through each entity's data file
    ...
    return closest_entity, min_distance, closest_entity_pos, player_angle, nearest_entity_angle


def find_all_entity_within_three_radius(player_file, entity_files):
    # Read the last row of the player's data file
    last_frame_player = find_last_row_of_csv(player_file)
    player_pos = (last_frame_player['posX'][0], last_frame_player['posY'][0])
    player_angle = last_frame_player['angle'][0]
...
for player_file, entity_files in player_entity_pairs.items():
    closest_entity, distance, entity_position, player_angle, entity_angle = find_closest_entity(player_file,
                                                                                                entity_files)
    find_all_entity_within_three_radius(player_file, entity_files)
...
entities_df.to_csv('entity_within_three_radius.csv', index=False)
